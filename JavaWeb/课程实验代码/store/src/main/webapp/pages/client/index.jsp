	...
	<Script type="text/javascript">
		$(function () {
			...
			$("button.addToCart").click(function () {
				...
				location.href = "http://localhost:8080/store/cartServlet?action=addItem&id=" + bookId;
			});
		});
	</Script>
	...
	<div id="main">
		<div id="book">
			...
			<div style="text-align: center">
				<span>您的购物车中有
					<c:choose>
						<c:when test="${not empty sessionScope.cart.totalCount}">
				...
				<div>
					...
					<c:choose>
						<c:when test="${sessionScope.lastName!=null && sessionScope.cart.totalCount>0}">
							您刚刚将<span style="color: red">${sessionScope.lastName}</span>加入到了购物车中
						</c:when>
						<c:otherwise>
							您还没有将商品添加到购物车中,快来添加吧!
						</c:otherwise>
					</c:choose>
			...
			<div class="b_list">
				...
				<div class="book_add">
					<button bookId="${good.id}" class="addToCart">加入购物车</button>
				</div>
				</div>
			</div>
			</c:forEach>
